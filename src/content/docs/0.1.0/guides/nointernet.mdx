---
title: Running sf-mouse without internet access
description: How to run sf-mouse with no internet access
...
<Steps>
  ...
  4. Run the `nf-core pipelines download`
     ...
     <CommandOutputs>
       <span slot="command">
         ```bash
         nf-core pipelines download scilus/sf-mouse -r 0.1.0 -o ./containers/ -s singularity -l docker.io -u copy --force
         ```
       </span>
...
Now you are good to go! Please refer to the [usage documentation on how to launch the pipeline](/sf-mouse/0.1.0/guides/usage).
...
Some `sf-mouse` core functionalities are accessed and selected using profiles and arguments. 
...
<Steps>
  1. **`-pre_qc`:**
     ...
     :::caution
     **If you want to use the output of the **pre QCed** data you must add --use_preqc. See [parameters section](/sf-mouse/0.1.0/guides/parameters).**
     :::
...
<Tabs>
  <TabItem label="Command">
    ```bash
    nextflow run scilus/sf-mouse -r 0.1.0 --input <input_directory> --outdir ./results --use_preqc true -profile docker,pre_qc -resume
    ```
  </TabItem>
</Tabs>
...
:::note
More parameters can be tuned to your own usage, for a concise list of the most common parameters, you can run `nextflow run scilus/sf-mouse -r 0.1.0 --help`. Otherwise, for the complete list of parameters, please refer to the [parameters section](/sf-mouse/0.1.0/guides/parameters) of the documentation.
:::
...
<FileTree>
  ...
  * sf-mouse-v0.1.0/            # Finished results in specified location (defined with --outdir)
    * sub-01
        * anat/
        * dwi/
        * stats/
...
<Tabs>
  <TabItem label="Command">
    ```bash
    nextflow run scilus/sf-mouse -r 0.1.0 -profile docker -params-file params.yaml
    ```
  </TabItem>
</Tabs>
